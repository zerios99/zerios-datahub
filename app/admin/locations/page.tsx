...
function AdminLocationsContent() {
  ...
  const [deleteModal, setDeleteModal] = useState<{ show: boolean; locationId: string; locationName: string }>({
    show: false,
    locationId: '',
    locationName: ''
  });
  const [approveModal, setApproveModal] = useState<{ show: boolean; locationId: string; locationName: string }>({
    show: false,
    locationId: '',
    locationName: ''
  });
  const [rejectModal, setRejectModal] = useState<{ show: boolean; locationId: string; locationName: string }>({
    show: false,
    locationId: '',
    locationName: ''
  });
  ...
  const handleApprove = async () => {
    try {
      const response = await fetch(`/api/admin/locations/${approveModal.locationId}`, {
        method: 'PATCH',
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(`/api/admin/locations/${rejectModal.locationId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteLocation = async () => {
    try {
      const response = await fetch(`/api/admin/locations/${deleteModal.locationId}`, {
        method: 'DELETE',
      });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'APPROVED':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 ring-1 ring-green-600/20 dark:ring-green-400/30">Approved</span>;
      case 'REJECTED':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 ring-1 ring-red-600/20 dark:ring-red-400/30">Rejected</span>;
      default:
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 ring-1 ring-yellow-600/20 dark:ring-yellow-400/30">Pending</span>;
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <div className="lg:ml-64 p-4 sm:p-6 lg:p-8">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 lg:mb-8">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Location Management</h2>
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mt-1">Manage and review user-submitted locations</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 lg:gap-6 mb-6 lg:mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 sm:p-6 border-l-4 border-blue-500 dark:border-blue-400">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">{stats.total}</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 sm:p-6 border-l-4 border-yellow-500 dark:border-yellow-400">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">{stats.pending}</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 sm:p-6 border-l-4 border-green-500 dark:border-green-400">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">{stats.approved}</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 sm:p-6 border-l-4 border-red-500 dark:border-red-400">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">{stats.rejected}</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 sm:p-6 border-l-4 border-purple-500 dark:border-purple-400">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 font-medium">Sponsored</p>
                <p className="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white mt-1">{stats.sponsored}</p>
              </div>
        ...
        {loadingLocations ? (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-12 text-center border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-sm sm:text-base text-gray-500 dark:text-gray-400 mt-4">Loading locations...</p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-12 text-center border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              {searchQuery ? `No locations found matching "${searchQuery}"` : 'No locations found'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            {filteredLocations.map((location) => (
              <div key={location.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-shadow">
                  ...
                  <div className="flex-1 p-4">
                    <div className="flex items-start justify-between mb-2 gap-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">{location.city}</p>
                      </div>
                    ...
                    <div className="text-xs text-gray-600 dark:text-gray-400 mb-3 space-y-1">
                      ...
                      {location.formalPlaceName && (
                        <div className="truncate">
                          <span className="font-medium">Formal:</span> {location.formalPlaceName}
                        </div>
                      )}
                      {location.street && (
                        <div className="truncate">
                      ...
                      {location.pointType && (
                        <div>
                          <span className="font-medium">Type:</span> {location.pointType === 'new' ? '🆕 New' : '✏️ Edit'}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {location.status === 'PENDING' && (
                        <>
                          <button
                            onClick={() => setApproveModal({ show: true, locationId: location.id, locationName: location.name })}
                          ...
                          <button
                            onClick={() => setRejectModal({ show: true, locationId: location.id, locationName: location.name })}
                      ...
                      <button
                        onClick={() => setDeleteModal({ show: true, locationId: location.id, locationName: location.name })}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in duration-200">
            <div className="p-6">
              ...
              <h3 className="text-lg font-bold text-gray-900 dark:text-white text-center mb-2">Approve Location</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400 text-center mb-6">
                Are you sure you want to approve <span className="font-semibold text-gray-900 dark:text-white">"{approveModal.locationName}"</span>?
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => setApproveModal({ show: false, locationId: '', locationName: '' })}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in duration-200">
            <div className="p-6">
              ...
              <h3 className="text-lg font-bold text-gray-900 dark:text-white text-center mb-2">Reject Location</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400 text-center mb-6">
                Are you sure you want to reject <span className="font-semibold text-gray-900 dark:text-white">"{rejectModal.locationName}"</span>?
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => setRejectModal({ show: false, locationId: '', locationName: '' })}
                  className="flex-1 px-4 py-2.5 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors font-medium"
                >
                  Cancel
                </button>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in duration-200">
            <div className="p-6">
              ...
              <h3 className="text-lg font-bold text-gray-900 dark:text-white text-center mb-2">Delete Location</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400 text-center mb-6">
                Are you sure you want to delete <span className="font-semibold text-gray-900 dark:text-white">"{deleteModal.locationName}"</span>? This action cannot be undone.
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => setDeleteModal({ show: false, locationId: '', locationName: '' })}
                  className="flex-1 px-4 py-2.5 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors font-medium"
                >
                  Cancel
                </button>
